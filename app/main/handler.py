...
def prep_bottle_form(user, form: Union[BottleForm, BottleEditForm]) -> Union[BottleForm, BottleEditForm]:
    ...
    # set up the bottlers dropdown (same as distilleries)
    bottlers = user.distilleries
    bottlers.sort(key=lambda d: d.name)
    form.bottler_id.choices = [(d.id, d.name) for d in distilleries]
    form.bottler_id.choices.insert(0, ("", "Choose a Bottler"))
...
def bottle_add_images(form: Union[BottleForm, BottleEditForm], bottle_in: Bottle) -> bool:
    for i in range(1, 4):
...
def bottle_delete_images(bottle: Bottle):
    s3_client = boto3.client("s3")
    for i in range(1, 4):
        s3_client.delete_object(Bucket="my-whiskies-pics", Key=f"__del_{bottle.id}_{i}.png")


def get_bottle_image_count(bottle_id: str) -> int:
    s3_client = boto3.client("s3")
    images = s3_client.list_objects(Bucket="my-whiskies-pics", Prefix=bottle_id).get("Contents", [])
    return len(images)
