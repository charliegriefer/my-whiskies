...
@bottle_bp.route("/bottle/diagnostics", endpoint="diagnostics")
@login_required
def bottle_diagnostics():
    """Temporary diagnostic route to help debug image issues"""
    # Get a sample of bottles
    bottles = (
        db.session.execute(
            db.select(Bottle)
    ...
    for bottle in bottles:
        bottle_info = {
            "id": bottle.id,
            "name": bottle.name,
            "image_count_property": bottle.image_count,
            "raw_images_length": len(bottle.images),
            "image_details": [
                {"id": img.id, "sequence": img.sequence, "created_at": img.created_at}
                for img in bottle.images
            ],
        }
        image_info.append(bottle_info)

    return render_template(
        "bottle/diagnostics.html",
        title="Image Diagnostics",
        bottles=bottles,
        image_info=image_info,
        debug_info={"s3_config": get_s3_config()},
    )
