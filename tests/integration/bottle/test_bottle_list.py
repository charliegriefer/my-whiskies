...
def test_bottle_list(client: FlaskClient, test_user_01: User) -> None:
    ...
    response = client.get(url_for("bottle.bottles", username=test_user_01.username))
...
def test_bottle_list_logged_in_elements(
    ...
    response = logged_in_user_01.get(
        url_for("bottle.bottles", username=test_user_02.username)
    )
    response_data = response.get_data(as_text=True)
    assert response.status_code == 200
    ...
    response = logged_in_user_01.get(
        url_for("bottle.bottles", username=test_user_01.username)
    )
    response_data = response.get_data(as_text=True)
    assert response.status_code == 200
