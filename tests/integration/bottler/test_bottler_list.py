...
def test_bottler_list(client: FlaskClient, test_user_01: User):
    ...
    response = client.get(url_for("bottler.list", username=test_user_01.username))
...
def test_bottler_list_logged_in_elements(
    ...
    response = client.get(url_for("bottler.list", username=test_user_02.username))
    assert response.status_code == 200
...
def test_bottler_list_logged_out_elements(client: FlaskClient):
    ...
    response = client.get(url_for("bottler.list", username="testuser"))
    response_data = response.get_data(as_text=True)
